...
std::int32_t DeterministicRng::nextI32(std::int32_t minInclusive,
                                       std::int32_t maxInclusive) noexcept {
    ...
    const std::uint32_t span =
        static_cast<std::uint32_t>(static_cast<std::int64_t>(maxInclusive) -
                                   static_cast<std::int64_t>(minInclusive) + 1);

    const std::uint32_t value = nextU32();
    const std::uint32_t offset = (span == 0U) ? value : (value % span);

    return static_cast<std::int32_t>(static_cast<std::int64_t>(minInclusive) +
